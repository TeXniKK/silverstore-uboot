...
//libo 2010-06-30
	//mvBoardDebugLed(4);
//for topkick1280 mpp49
//andrew start
//libo 2011-03-28
        //mvGppTypeSet(1, MV_GPP17, (MV_GPP_OUT & MV_GPP17)); //mvGppTypeSet(1, BIT2, (MV_GPP_OUT & BIT2));
        //mvGppBlinkEn(1, MV_GPP17, (MV_GPP_OUT_BLINK & MV_GPP17));
        unsigned int Val;
//mpp21
#if 0 //libo 2011-03-01
        unsigned int Val;
        Val = MV_REG_READ(mvCtrlMppRegGet(2));
        Val &= 0xff0fffff;
        MV_REG_WRITE(mvCtrlMppRegGet(2), Val);
	...
	Val = MV_REG_READ(0x8002C); //SATAHC LED config reg
	MV_REG_WRITE(0x8002C, Val | BIT0);
...
//			 ubootSize, nand_info[0].size - 0x100000, 0x100000);
		sprintf(buff,"console=ttyS0,115200");
		env = getenv("console");
		if(!env)
			setenv("console",buff);
...
setenv bootargs $(console) $(bootargs_root) nfsroot=$(serverip):$(rootpath) \
ip=$(ipaddr):$(serverip)$(bootargs_end) $(mvNetConfig) $(mvPhoneConfig);  bootm 0x2000000; ");
            setenv("bootcmd","nand read.e 0x1200000 0x200000 0x600000;nand read.e 0x2000000 0x800000 0x1000000;bootm 0x1200000 0x2000000");
        env = getenv("bootargs");
       if(!env)
